import { animationSettings, wallPhase, presentationPhase, videoSource } from './wall-timing.js?v=menu-controls-1';
const app=document.querySelector('#app');
let menu,view='boards',selected=1,editing=null,addingScreenId=null;
...
function board(screen){const items=menu.items.filter(i=>i.screenId===screen.id&&i.visible).sort((a,b)=>a.sortOrder-b.sortOrder);const promos=menu.promotions.filter(p=>p.screenId===screen.id&&p.visible).sort((a,b)=>a.sortOrder-b.sortOrder);return `<section class="board ${e(screen.layout)}" aria-label="TV ${screen.physicalPosition}: ${e(screen.name)}"><div class="board-head"><span>${e(menu.brand.name)}</span><span>${e(screen.name)}</span></div><div class="board-items">${!screen.visible?'<p class="empty">Menu temporarily unavailable</p>':items.map(i=>{const asset=menu.assets.find(a=>a.id===i.imageAssetId);return `<article class="item ${i.available?'':'is-unavailable'}"${i.available?'':` aria-label="${e(i.name)}, not available"`}><h2>${e(i.name)}</h2>${i.price!==null?`<div class="price">${money(i.price)}</div>`:''}${asset?.src?`<div class="image-wrap"><img class="food" src="${e(asset.src)}" alt="${e(i.name)}"></div>`:'<div class="placeholder">Photo to be supplied</div>'}${i.variants.map(v=>`<div class="variant"><span>${e(v.label)}</span><strong>${money(v.price)}</strong></div>`).join('')}${i.combo?`<div class="combo">${e(i.combo.label)} +${money(i.combo.priceDelta)}</div>`:''}${i.description?`<p class="description">${e(i.description)}</p>`:''}${!i.available?'<div class="unavailable">Not available</div>':''}</article>`}).join('')}</div>${screen.visible&&promos.length?`<div class="board-footer">${promos.map(p=>`<span>${e(p.text)}</span>`).join('')}</div>`:''}</section>`}
...
function newItem(screenId){const screen=menu.screens.find(s=>s.id===screenId);return {name:'',category:screen.name,price:null,currency:menu.currency,description:'',visible:true,available:true,screenId,sortOrder:Math.max(0,...menu.items.filter(i=>i.screenId===screenId).map(i=>i.sortOrder))+1,variants:[],combo:null,addOns:[],imageAssetId:null,source:{file:screen.sourceFile,confidence:'owner-added'}}}
function itemRows(screen){const items=menu.items.filter(i=>i.screenId===screen.id).sort((a,b)=>a.sortOrder-b.sortOrder);return items.length?items.map(i=>`<tr><td>${e(i.name)}<small>${e(i.category)}</small></td><td>${i.price!==null?money(i.price):i.variants.map(v=>e(v.label)+' '+money(v.price)).join(' / ')||'—'}${i.combo?`<small>${e(i.combo.label)} +${money(i.combo.priceDelta)}</small>`:''}</td><td><span class="status-pill ${!i.visible?'hidden':i.available?'available':'unavailable-status'}">${!i.visible?'Hidden':i.available?'Available':'Not available'}</span></td><td><button data-edit="${e(i.id)}">Edit<span class="source-name"> ${e(i.name)}</span></button></td></tr>`).join(''):`<tr><td colspan="4" class="empty-row">No items on this screen yet.</td></tr>`}
function renderAdmin(){
  const draft=addingScreenId?newItem(addingScreenId):menu.items.find(i=>i.id===editing);
  app.innerHTML=`<div class="heading"><div><h1>Edit menu</h1><p class="muted">${menu.items.length} products · Add, edit or remove items from each screen.</p></div><label class="actions">Import menu<input id="import" type="file" accept="application/json" style="width:220px"></label></div><div class="notice">Changes apply to this preview session. Export the menu to keep them. Menu edits still need to be exported and published to reach remote TVs.</div>${draft?editor(draft,Boolean(addingScreenId)):''}<div class="screen-editor-grid">${[...menu.screens].sort((a,b)=>a.physicalPosition-b.physicalPosition).map(screen=>`<section class="panel screen-editor"><div class="panel-heading"><div><h2>TV ${screen.physicalPosition} · ${e(screen.name)}</h2><p class="muted">${menu.items.filter(i=>i.screenId===screen.id).length} items</p></div><button class="primary" data-add="${screen.id}">Add item</button></div><div class="scroll-table"><table><thead><tr><th>Product</th><th>Price / options</th><th>Status</th><th></th></tr></thead><tbody>${itemRows(screen)}</tbody></table></div></section>`).join('')}</div><div class="panel"><h2>Screen mapping</h2><p class="muted">Move a whole board by changing its physical TV position. Individual products can move independently.</p><form id="mapping">${menu.screens.map(s=>`<label>${e(s.name)} · ${e(s.sourceFile)}<select name="s${s.id}">${[1,2,3,4].map(n=>`<option ${n===s.physicalPosition?'selected':''}>${n}</option>`).join('')}</select></label>`).join('')}<button class="primary" type="submit">Apply mapping</button></form></div><div class="panel"><h2>Complete menu data</h2><p class="muted">Edit promotions, category labels, sizes, descriptions, add-ons, assets, screen names and layouts here. Unknown values stay null.</p><details><summary>Open structured data editor</summary><form id="rawform"><label class="full">Menu JSON<textarea id="raw" rows="20" spellcheck="false">${e(JSON.stringify(menu,null,2))}</textarea></label><button class="primary">Apply menu data</button></form></details></div>`;
  app.querySelectorAll('[data-edit]').forEach(b=>b.onclick=()=>{editing=b.dataset.edit;addingScreenId=null;renderAdmin();app.querySelector('#itemform').scrollIntoView({behavior:'smooth',block:'start'})});
  app.querySelectorAll('[data-add]').forEach(b=>b.onclick=()=>{editing=null;addingScreenId=Number(b.dataset.add);renderAdmin();app.querySelector('#itemform').scrollIntoView({behavior:'smooth',block:'start'})});
  app.querySelector('#mapping').onsubmit=ev=>{ev.preventDefault();const f=new FormData(ev.target),positions=menu.screens.map(s=>Number(f.get('s'+s.id)));if(new Set(positions).size!==4)return notify('Give each board a different TV position.');menu.screens.forEach((s,n)=>s.physicalPosition=positions[n]);render();notify('Screen mapping updated.');};
  app.querySelector('#rawform').onsubmit=ev=>{ev.preventDefault();try{const next=JSON.parse(app.querySelector('#raw').value);validate(next);menu=next;editing=null;addingScreenId=null;render();notify('Menu data applied.');}catch(err){notify('Could not apply: '+err.message)}};
  app.querySelector('#import').onchange=async ev=>{try{const file=ev.target.files[0];if(!file)return;const next=JSON.parse(await file.text());validate(next);menu=next;editing=null;addingScreenId=null;render();notify('Menu imported.');}catch(err){notify('Could not import: '+err.message)}};
  if(draft)bindEditor(draft,Boolean(addingScreenId));
}
function editor(i,isNew){const asset=menu.assets.find(a=>a.id===i.imageAssetId);return `<div class="panel item-editor"><div class="panel-heading"><div><p class="eyebrow">${isNew?'New menu item':'Edit item'}</p><h2>${e(i.name)||'Add an item'}</h2></div>${!isNew?'<button type="button" id="delete-item" class="danger">Delete item</button>':''}</div><form id="itemform"><label>Product name<input name="name" required value="${e(i.name)}"></label><label>Category<input name="category" required value="${e(i.category)}"></label><label>Price (leave blank for sizes)<input type="number" name="price" min="0" step="0.01" value="${i.price??''}"></label><label>Screen<select name="screenId">${screenOptions(i.screenId)}</select></label><label>Order on screen<input type="number" name="sortOrder" min="1" required value="${i.sortOrder}"></label><label>Combo surcharge<input type="number" name="comboPrice" min="0" step="0.01" value="${i.combo?.priceDelta??''}"></label><label class="full">Description<textarea name="description">${e(i.description)}</textarea></label><label class="full picture-field">Item picture<input type="file" name="picture" accept="image/png,image/jpeg,image/webp"><span class="field-help">PNG, JPG or WebP, up to 5 MB. Choosing a file replaces the current picture.</span></label>${asset?.src?`<div class="full current-picture"><img src="${e(asset.src)}" alt="Current picture for ${e(i.name)}"><label class="check"><input type="checkbox" name="removePicture">Remove current picture</label></div>`:''}${i.variants.map((v,n)=>`<label>${e(v.label)} price<input type="number" min="0" step="0.01" required name="variant${n}" value="${v.price}"></label>`).join('')}<label class="check"><input type="checkbox" name="visible" ${i.visible?'checked':''}>Visible on menu</label><label class="check"><input type="checkbox" name="available" ${i.available?'checked':''}>Available</label><div class="full actions"><button class="primary" type="submit">${isNew?'Add item':'Apply changes'}</button><button type="button" id="cancel">Cancel</button><span class="muted">${isNew?'This item will be added to the selected screen.':'Source: '+e(i.source.file)}</span></div></form></div>`}
function uniqueId(name){const base=name.toLowerCase().normalize('NFKD').replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')||'menu-item';let id=base,n=2;while(menu.items.some(i=>i.id===id)){id=`${base}-${n++}`}return id}
function pictureData(file){return new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=()=>reject(Error('The picture could not be read.'));reader.readAsDataURL(file)})}
function bindEditor(draft,isNew){
  app.querySelector('#cancel').onclick=()=>{editing=null;addingScreenId=null;renderAdmin()};
  if(!isNew)app.querySelector('#delete-item').onclick=()=>{if(!confirm(`Delete ${draft.name} from the menu?`))return;menu.items=menu.items.filter(i=>i.id!==draft.id);editing=null;renderAdmin();notify('Item deleted from the menu preview.');};
  app.querySelector('#itemform').onsubmit=async ev=>{ev.preventDefault();const f=new FormData(ev.target),picture=f.get('picture');if(picture?.size&&!['image/png','image/jpeg','image/webp'].includes(picture.type))return notify('Choose a PNG, JPG or WebP picture.');if(picture?.size>5*1024*1024)return notify('Choose a picture smaller than 5 MB.');try{const pictureSrc=picture?.size?await pictureData(picture):null,i=isNew?draft:menu.items.find(i=>i.id===editing);i.name=f.get('name').trim();i.category=f.get('category').trim();i.description=f.get('description').trim();i.price=f.get('price')===''?null:Number(f.get('price'));i.screenId=Number(f.get('screenId'));i.sortOrder=Number(f.get('sortOrder'));i.visible=f.has('visible');i.available=f.has('available');i.variants.forEach((v,n)=>v.price=Number(f.get('variant'+n)));i.combo=f.get('comboPrice')===''?null:{...(i.combo||{label:'COMBO!',includes:null}),priceDelta:Number(f.get('comboPrice'))};if(isNew){i.id=uniqueId(i.name);i.imageAssetId=i.id;menu.assets.push({id:i.id,label:`${i.name} photo`,status:'needed',src:null});}const asset=menu.assets.find(a=>a.id===i.imageAssetId);if(f.has('removePicture')&&asset){asset.src=null;asset.status='needed';}if(pictureSrc&&asset){asset.src=pictureSrc;asset.status='provided';}if(isNew)menu.items.push(i);editing=null;addingScreenId=null;renderAdmin();notify(isNew?'Item added to the menu preview.':'Changes applied to the menu preview.');}catch(error){notify(error.message)}};
}
...
function renderWall() {
  ...
  const films=config.playlist?.filter(film=>film.enabled!==false)||[config.video];
  ...
  app.innerHTML=`<div class="heading"><div><h1>Shawarma animations across four TVs</h1>
    ...
    ${config.video?.src?`<div class="panel"><h2 id="film-title">${e(films[0]?.title||'Fire to Fez')} · Mr. Fez</h2><video class="clip-player" style="${config.mode==='panoramic-video'?'aspect-ratio:64/9;max-width:none':''}" controls playsinline preload="metadata" src="${e(films[0]?.src||config.video.src)}" aria-label="Play the selected Mr. Fez wall animation"></video><p class="muted"><a id="film-download" href="${e(films[0]?.src||config.video.src)}" download>Download the video</a> · ${config.video.durationSeconds} seconds · Includes your logo</p></div>`:''}
    ...
    ${config.playlist?`<fieldset class="full film-selection"><legend>Animations active in the rotation</legend>${config.playlist.map((film,index)=>`<label class="check"><input type="checkbox" name="film-${index}" ${film.enabled!==false?'checked':''}>${e(film.title)}</label>`).join('')}<p class="muted">Select one or more. The TVs rotate through only the selected animations.</p></fieldset>`:''}
...
function startEffects() {
  ...
  function tick() {
    ...
    const activeFilms=config.playlist?.filter(film=>film.enabled!==false);
    const message=off?'Animation off on this device (motion preference).':!config.enabled?'Animation disabled.':demo?'Local demo · remote TVs keep their shared schedule':slot.kind==='video'?`${activeFilms?.[slot.clipIndex]?.title||'Wall animation'} · ${Math.ceil(slot.remainingMs/1000)} seconds remaining${targets.some(t=>t.blocked||t.video?.error)?' · Video unavailable on a player; menu retained':''}`:slot.kind==='flame'?`Live flame effect · ${Math.ceil(slot.remainingMs/1000)} seconds remaining`:`Shared schedule · next animation in ${Math.ceil(slot.remainingMs/1000)} seconds`;
