...
function bindEditor(draft,isNew){
  ...
  app.querySelector('#itemform').onsubmit=ev=>{ev.preventDefault();const f=new FormData(ev.target),i=isNew?draft:menu.items.find(i=>i.id===editing);i.name=f.get('name').trim();i.category=f.get('category').trim();i.description=f.get('description').trim();i.price=f.get('price')===''?null:Number(f.get('price'));i.screenId=Number(f.get('screenId'));i.sortOrder=Number(f.get('sortOrder'));i.visible=f.has('visible');i.available=f.has('available');i.variants.forEach((v,n)=>v.price=Number(f.get('variant'+n)));i.combo=f.get('comboPrice')===''?null:{...(i.combo||{label:'COMBO!',includes:null}),priceDelta:Number(f.get('comboPrice'))};if(isNew){i.id=uniqueId(i.name);i.imageAssetId=i.id;menu.assets.push({id:i.id,label:`${i.name} photo`,status:'needed',src:null});menu.items.push(i);}editing=null;addingScreenId=null;renderAdmin();notify(isNew?'Item added to the menu preview.':'Changes applied to the menu preview.');};
}
